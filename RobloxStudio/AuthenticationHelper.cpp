...
bool AuthenticationHelper::eventFilter(QObject * watched, QEvent * evt)
{
	...
	{
        // save webkit cookies
		savedCookieKey = RobloxNetworkAccessManager::Instance().cookieJar()->getCookieValue(RobloxSettings::getBaseURL(), "shitass");
	}
	else if (eventType == QEvent::ApplicationActivate)
	{
		QString currentCookieKey = RobloxNetworkAccessManager::Instance().cookieJar()->getCookieValue(RobloxSettings::getBaseURL(), "shitass");
...
void AuthenticationHelper::authenticateUserAsync(const QString& url, const QString& ticket)
{
	...
	else
	{
		...
		QString secCookie = RobloxNetworkAccessManager::Instance().cookieJar()->getCookieValue(RobloxSettings::getBaseURL(), "shitass");
...
bool AuthenticationHelper::authenticateQtWebkitSession(const QString& authenticationUrl)
{
	...
	networkRequest.setRawHeader("RBXAuthenticationNegotiation:", baseURL.toLatin1());
	networkRequest.setRawHeader("User-Agent", userAgentStr.toLatin1());

...
namespace StudioUtilities
{
	...
	RBX::ProtectedString fetchAndValidateScript(RBX::DataModel* pDataModel, const std::string& urlScript)
	{
		...
		std::string dataString;
		{
			try 
			{
				...
				std::unique_ptr<std::istream> stream(RBX::ServiceProvider::create<RBX::ContentProvider>(pDataModel)->getContent(RBX::ContentId(urlScript.c_str())));
	...
	void insertScript(boost::shared_ptr<RBX::DataModel> pDataModel, const QString &fileName)
	{
		...
		QTextStream in(&file);
		pScriptInstance->setEmbeddedCode(RBX::ProtectedString::fromTrustedSource(in.readAll().toLatin1().data()));

		...
		QString scriptText;
		{
			...
			if(spScriptInstance)
			{
				...
				else
				{
					...
					if (pContentProvider)
					{
						std::unique_ptr<std::istream> stream = pContentProvider->getContent(spScriptInstance->getScriptId());
...
std::unique_ptr<XmlElement> SelectionSaveToFileVerb::writeSelection()
{

...
DYNAMIC_FASTSTRINGVARIABLE(WorkspaceMessageLink, std::string("http://www.finobe.net"))
...
void RobloxIDEDoc::teleportToURL(QString url, bool play)
{
    ...
    // Check to see if the url fetched returns anything at all.
    {
        ...
        std::unique_ptr<std::istream> stream = RBX::ServiceProvider::create<RBX::ContentProvider>(dataModel.get())->getContent(RBX::ContentId(command));

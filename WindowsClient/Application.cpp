...
{
	int crash = 0;
	if (RBX::RegistryUtil::read32bitNumber("HKEY_CURRENT_USER\\Software\\Finobe_Penelope\\Finobe\\InferredCrash", crash)) 
	{
		...
		points.report("Windows-FinobePlayer-SessionReport-Inferred", FInt::InferredCrashReportingHundredthsPercentage);
		RBX::Analytics::EphemeralCounter::reportCounter(crash ? "Windows-FinobePlayer-Session-Inferred-Crash" : "Windows-FinobePlayer-Session-Inferred-Success", 1, true);
	}
	
	RBX::RegistryUtil::write32bitNumber("HKEY_CURRENT_USER\\Software\\Finobe_Penelope\\Finobe\\InferredCrash", 1);
}
...
void Application::UploadSessionLogs()
{
	if(!logManager.CreateFakeCrashDump())
	{
		...
		MessageBoxA(mainWindow, 
			"Logfiles will be uploaded next time you start Finobe. Please restart finobe now.", 
			"Finobe", MB_OK);
	}
}
...
static void doMachineIdCheck(Application* app, FunctionMarshaller* marshaller, HWND hWnd) {
	if (MachineIdUploader::uploadMachineId(GetBaseURL().c_str()) ==
		...
		MessageBoxA(hWnd, MachineIdUploader::kBannedMachineMessage, "Finobe", MB_OK);
...
bool Application::LoadAppSettings(HINSTANCE hInstance)
{
	try {
		...
		CRegKey key;
		bool validRegKey = (key.Open(HKEY_CURRENT_USER, "Software\\Finobe_Penelope\\Finobe", KEY_READ) == ERROR_SUCCESS);
		if (validRegKey)
		{
			DWORD value = TRUE;
	...
	catch(const std::exception& e)
	{
		...
		RBX::StandardOut::singleton()->printf(RBX::MESSAGE_ERROR, message.c_str());
		return false;
	}
}
...
void Application::Shutdown()
{
	...
	RBX::RegistryUtil::write32bitNumber("HKEY_CURRENT_USER\\Software\\Finobe_Penelope\\Finobe\\InferredCrash", 0);
...
void Application::uploadCrashData(bool userRequested)
{
	...
	if (!IsNetworkAlive(&result) || result == 0)
	{
		...
		if (userRequested) {
			MessageBoxA(mainWindow, 
				"Feature not available in offline mode.\nPlease check internet connection and restart Finobe.",
		...
		case RBX::DebugSettings::Prompt: 
			{
				...
				if (MessageBoxA(mainWindow, message, "Finobe", MB_YESNO)==IDYES)
					dumpErrorUploader->Upload(dmpHandlerUrl);
				else
...
void Application::handleError(const std::exception& e)
{
	...
	if (!RobloxCrashReporter::silent) {
		MessageBoxA(NULL, e.what(), "Finobe", MB_OK | MB_ICONSTOP);
	}
}

void Application::waitForNewPlayerProcess(HWND hWnd)
{
	static const char kPreventMultipleRobloxPlayersEventName[] = "Finobe_singletonEvent";
	static const char kPreventMultipleRobloxPlayersMutexName[] = "Finobe_singletonMutex";

...
void BootstrapperRccService::installService()
{
	...
	const TCHAR rbxRegPath[] = _T("Software\\Finobe_Penelope");
	CRegKey pathKey;
	pathKey.Create(HKEY_LOCAL_MACHINE, rbxRegPath);
	if (pathKey.m_hKey == NULL)
		throw std::runtime_error("Unable to create/open reg key: 'Software\\Finobe_Penelope'");

	LONG res = pathKey.SetStringValue(_T("RccServicePath"), (LPCTSTR)ProgramDirectory().c_str());
	if (res != ERROR_SUCCESS)
		throw std::runtime_error("Failed writing to registry: 'Software\\Finobe_Penelope\\RccServicePath'");
}

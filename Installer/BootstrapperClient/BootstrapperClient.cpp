...
static const TCHAR* BootstrapperFileName    = _T("PenelopeLauncher.exe");
static const TCHAR* RobloxAppFileName		= _T(PLAYEREXENAME);
static const TCHAR* BootstrapperMutexName   = _T("www.finobe.net/bootstrapper");
static const TCHAR* StartRobloxAppMutex     = _T("www.finobe.net/startRobloxApp");
static const TCHAR* LauncherFileName        = _T("RobloxProxy.dll");
static const TCHAR* LauncherFileName64      = _T("RobloxProxy64.dll");
static const TCHAR* FriendlyName            = _T("Finobe");
...
LPCWSTR robloxUpdaterTaskName = L"FinobeGameUpdater";
...
bool BootstrapperClient::NeedPreDeployRun()
{
	...
	CRegKey key;
	if (SUCCEEDED(key.Open(HKEY_CURRENT_USER, _T("Software\\Finobe_Penelope"), KEY_READ)))
	{
		TCHAR buf[MAX_PATH];
...
void BootstrapperClient::RunPreDeploy()
{
	...
	try
	{
		...
		CRegKey key;
		if (SUCCEEDED(key.Create(HKEY_CURRENT_USER, _T("Software\\Finobe_Penelope"))))
		{
...
bool BootstrapperClient::ProcessProtocolHandlerArgs(const std::map<std::wstring, std::wstring>& argMap)
{
	...
	if (launchMode == _T("play"))
	{
		...
		playArgs->authUrl = "https://" + BaseHost() + "/penelope/login/negotiate";
...
void BootstrapperClient::StartRobloxApp(bool fromInstall)
{
	...
	message("Finobe is up to date.");

	LOG_ENTRY("Creating event");
	CEvent robloxStartedEvent(NULL, TRUE, FALSE, _T("www.finobe.net/robloxStartedEvent"));
	...
	if (!playArgs)
	{
		...
		if (!isIDE)
		{
			std::wstring url = format_string(_T("%S%s"), BaseHost().c_str(), _T("/app/places"));;
...
void BootstrapperClient::deployRobloxProxy(bool commitData)
{
...

	{
		...
		CreateKey(allowedDomainsKey, _T("finobe.net"));
...
void BootstrapperClient::DeployComponents(bool isUpdating, bool commitData)
{
	...
	files.push_back(std::pair<std::wstring, std::wstring>(_T("Executable.zip"), _T("")));
	files.push_back(std::pair<std::wstring, std::wstring>(_T("Libraries.zip"), _T("")));
	files.push_back(std::pair<std::wstring, std::wstring>(_T("LibrariesQT4.zip"), _T("")));
	files.push_back(std::pair<std::wstring, std::wstring>(_T("content.zip"), _T("content")));

...
void CProgressDialog::OnCreate(HWND hWnd)
{
	...
	// set up button, start visible
	int bntWidth = 130;
	int bntHeight = 44;
	hWndButton = CreateWindowEx(0, L"Button", L"", WS_CHILD | WS_VISIBLE | BS_FLAT | BS_BITMAP,
...
void CProgressDialog::DisplayError(const char* message, const char* exceptionText)
{
	...
	MessageBox(convert_s2w(text).c_str(), _T("Finobe"), MB_OK | MB_ICONERROR);
}

void CProgressDialog::FinalMessage(const char* message)
{
	MessageBox(convert_s2w(message).c_str(), _T("Finobe"), MB_OK);
}

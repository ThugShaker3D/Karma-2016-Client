...
public:
	CSimpleModule() {}
	HRESULT AddCommonRGSReplacements(IRegistrarBase* pRegistrar) noexcept override
	{
		return S_OK;
	}
...
void Bootstrapper::RegisterUninstall(const TCHAR *productName)
{
	...
	throwHRESULT (keyProductCode.SetStringValue(_T("URLInfoAbout"), _T("http://www.finobe.net")), "Failed to set URLInfoAbout key");
		...
		if (found)
		{
			...
			if (retried)
			{
				int res = dialog->MessageBox(
					_T("Finobe might not launch correctly if Chrome is open or running in the background during installation.\n\n")
...
bool Bootstrapper::checkBootstrapperVersion()
{
	...
	if (moduleVersionNumber != bootstrapperVersion)
	{
		...
		if(moduleVersionNumber != bootstrapperVersion){
			...
			try
			{
				message("Getting the latest Finobe installer...");
				...
				newBootstrapper = simple_logger<wchar_t>::get_temp_filename(_T("tmp"));		// We could use an "exe" extension, but hiding the type isn't a bad idea?
				{
					...
					HttpTools::httpGetCdn(this, installHost, format_string("/%s-PublicLauncher.exe", installVersion.c_str()), std::string(), bootstrapperFile, false, boost::bind(&Bootstrapper::dummyProgress, _1, _2));
				}
...
void Bootstrapper::checkOSPrerequisit()
{
	...
	if (windowed)
		dialog->DisplayError("Finobe requires Microsoft Windows XP SP1 or greater", NULL);
...
void Bootstrapper::checkCPUPrerequisit()
{
    if (!Bootstrapper::hasSse2())
	{
	    ...
	    if (windowed)
		    dialog->DisplayError("Finobe requires SSE2 support", NULL);
...
void Bootstrapper::checkDirectXPrerequisit()
{
	...
	{
		LOG_ENTRY("checkDirectXPrerequisit failed");
		if (windowed)
			dialog->DisplayError("Finobe requires DirectX 9.0 or greater", NULL);
...
void Bootstrapper::checkDiskSpace()
{
	...
	{
		dialog->DisplayError("There is not enough room on your disk to install Finobe. Please free up some space and try again.", NULL);
...
void Bootstrapper::run()
{
	...
	try
	{
		...
		if (!IsNetworkAlive(&result) || result == 0)
		{
			...
			{
				CString message = _T("Finobe cannot connect to the Internet\n\nDoes your computer have a working network connection?  Is antivirus software preventing Finobe from accessing the Internet?");
				if (!robloxAppArgs.empty())
				{
					message += _T("\n\nIf you continue Finobe may not work properly.");
		...
		if (!checkBootstrapperVersion())
		{
			...
			if (windowed && isLatestProcess())
			{
				CString message = _T("Cannot connect to the Finobe website.\n\nIs antivirus software preventing Finobe from accessing the Internet?");
				if (!robloxAppArgs.empty())
				{
					message += _T("\n\nIf you continue Finobe may not work properly.");
					if (dialog->MessageBox(message, _T("Error"), MB_OKCANCEL | MB_ICONEXCLAMATION)==IDOK)
		...
		if(isFailIfNotUpToDate) 
		{
			...
			LOG_ENTRY("Finobe is up to date, returning success");
			goto done;
		}
...
void Bootstrapper::setLatestProcess()
{
	...
	const std::string latestProcessName = format_string("www.finobe.net/%s/%s/latestProcess", installHost.c_str(), Type().c_str());

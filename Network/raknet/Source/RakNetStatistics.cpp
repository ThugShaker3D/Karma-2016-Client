...
void RAK_DLL_EXPORT RakNet::StatisticsToString(const RakNetStatistics *s, char *buffer, int verbosityLevel )
{
	...
	{
		sprintf(buffer,
			"Actual bytes per second sent       \n"
			"Actual bytes per second received   \n"
			"Message bytes per second pushed    \n"
			"Total actual bytes sent            \n"
			"Total actual bytes received        \n"
			"Total message bytes pushed         \n"
			"Current packetloss                 %.1f%%\n"
		...
		if (s->BPSLimitByCongestionControl!=0)
		{
			char buff2[128];
			sprintf(buff2,
				"Send capacity                     bytes per second (%.0f%%)\n",
	...
	else
	{
		sprintf(buffer,
			"Actual bytes per second sent         \n"
			"Actual bytes per second received     \n"
			"Message bytes per second sent        \n"
			"Message bytes per second resent      \n"
			"Message bytes per second pushed      \n"
			"Message bytes per second processed   \n"
			"Message bytes per second ignored     \n"
			"Total bytes sent                     \n"
			"Total bytes received                 \n"
			"Total message bytes sent             \n"
			"Total message bytes resent           \n"
			"Total message bytes pushed           \n"
			"Total message bytes received         \n"
			"Total message bytes ignored          \n"
			"Messages in send buffer, by priority %i,%i,%i,%i\n"
			"Bytes in send buffer, by priority    %i,%i,%i,%i\n"
			"Messages in resend buffer            %i\n"
			"Bytes in resend buffer               \n"
			"Current packetloss                   %.1f%%\n"
			"Average packetloss                   %.1f%%\n"
			"Elapsed connection time in seconds   \n",
			(long long unsigned int) s->valueOverLastSecond[ACTUAL_BYTES_SENT],
